...
class GINEModel(nn.Module):
    def __init__(self, atom_in_features, edge_in_features, hidden_features, out_features, dropout_rate, activation_fn):
        ...
        # Используйте nn.Sequential для обработки признаков ребра в GINEConv
        ...
        # Первый и второй слои GINEConv
        self.gine_conv1 = GINEConv(edge_nn, train_eps=True)
        self.gine_conv2 = GINEConv(edge_nn, train_eps=True)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # Примените GINEConv отдельно к признакам узлов и рёбер
        atom_features = self.gine_conv1(atom_features, edge_index, edge_features)
        atom_features = self.bn1(atom_features)
        atom_features = self.activation(atom_features)
        atom_features = self.dropout(atom_features)

        atom_features = self.gine_conv2(atom_features, edge_index, edge_features)
        atom_features = self.bn2(atom_features)
        atom_features = self.activation(atom_features)
        atom_features = self.dropout(atom_features)

        out = self.postprocess(atom_features).squeeze(-1)
        return out
